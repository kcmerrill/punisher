...
func main() {
	...
	loopName := flag.String("loop", "", "Name of the loop to use")
	loopEnd := flag.Int("loop-ends-at", 0, "Size to end loop at (requires loop-name flag)")
	loopStartsAt := flag.Int("loop-starts-at", 0, "Starting value of where the loop should start at (requires loop-name flag)")
	loopIncrement := flag.Int("loop-increment-by", 1, "The amount to increment loop iterator by (requires loop-name flag)")
